...
export class TaskStateService {
   ...
   url = environment.api_buildboard + 'taskstate/';
   ...
   post(taskstate: TaskState): Promise<any> {
      return this.http.post(this.url, JSON.stringify(taskstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(taskstate: TaskState): Promise<any> {
      return this.http.put(this.url, JSON.stringify(taskstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
