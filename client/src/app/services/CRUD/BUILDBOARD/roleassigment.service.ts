...
export class RoleAssigmentService {
   ...
   url = environment.api_buildboard + 'roleassigment/';
   ...
   post(roleassigment: RoleAssigment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(roleassigment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(roleassigment: RoleAssigment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(roleassigment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
