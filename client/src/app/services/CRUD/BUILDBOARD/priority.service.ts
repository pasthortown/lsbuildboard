...
export class PriorityService {
   ...
   url = environment.api_buildboard + 'priority/';
   ...
   post(priority: Priority): Promise<any> {
      return this.http.post(this.url, JSON.stringify(priority), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(priority: Priority): Promise<any> {
      return this.http.put(this.url, JSON.stringify(priority), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
