...
export class RoleService {
   ...
   url = environment.api_buildboard + 'role/';
   ...
   post(role: Role): Promise<any> {
      return this.http.post(this.url, JSON.stringify(role), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(role: Role): Promise<any> {
      return this.http.put(this.url, JSON.stringify(role), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
