...
export class WorkTimeService {
   ...
   url = environment.api_buildboard + 'worktime/';
   ...
   post(worktime: WorkTime): Promise<any> {
      return this.http.post(this.url, JSON.stringify(worktime), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(worktime: WorkTime): Promise<any> {
      return this.http.put(this.url, JSON.stringify(worktime), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
