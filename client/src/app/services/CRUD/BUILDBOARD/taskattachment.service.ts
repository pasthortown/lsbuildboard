...
export class TaskAttachmentService {
   ...
   url = environment.api_buildboard + 'taskattachment/';
   ...
   post(taskattachment: TaskAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(taskattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(taskattachment: TaskAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(taskattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
