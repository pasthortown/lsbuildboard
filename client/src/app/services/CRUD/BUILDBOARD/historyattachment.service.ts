...
export class HistoryAttachmentService {

   url = environment.api_buildboard + 'historyattachment/';
   ...
   post(historyattachment: HistoryAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(historyattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(historyattachment: HistoryAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(historyattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
