...
export class ProjectCommentService {
   ...
   url = environment.api_buildboard + 'projectcomment/';
   ...
   post(projectcomment: ProjectComment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(projectcomment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(projectcomment: ProjectComment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(projectcomment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
