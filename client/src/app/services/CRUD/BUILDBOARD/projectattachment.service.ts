...
export class ProjectAttachmentService {
   ...
   url = environment.api_buildboard + 'projectattachment/';
   ...
   post(projectattachment: ProjectAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(projectattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(projectattachment: ProjectAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(projectattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
